...
router.get('/:election_id/:position_id/results', function (req, res, next) {
  ...
  var election = null;
  models.lection.findById(parseInt(req.params.election_id)).then(function(data) {
    election = data;
    ...
    include: [
      {
        ...
        as: "positions",
        include: {
          model: models.election_position_nominee,
          as: "nominees"
        }
      }
    ]
...
router.post('/:election_id/newposition', function (req, res, next) {
  models.election_position.create({
    name: req.body.name,
...
router.post('/:election_id/:position_id/newnominee', upload.single('manifesto'),function (req, res, next) {
  var moveManifestoPromise = new Promise(function(resolve, reject){
    if (req.file) {
      var manifestoName = slug(req.body.name)+'-'+shortid.generate()+'.'+mime.extension(req.file.mimetype);
      mv(req.file.path, __dirname+'/../../public/files/manifestos/'+manifestoName, function (err) {
        if(err) return reject(err);
        return resolve(manifestoName);
      });
    } else {
      return resolve(null);
    }
  ...
  moveManifestoPromise.then(function(manifestoName) {
